...
namespace AGS.Editor
{
    public partial class PreferencesEditor : Form
    {
        ...
        private void btnApplyColorTheme_Click(object sender, EventArgs e)
        {
            if (cmbColorTheme.SelectedItem is ColorThemeStub)
            {
                var s = (ColorThemeStub)cmbColorTheme.SelectedItem;
                Factory.Events.OnColorThemeLoaded("Default");
            }
            else if (cmbColorTheme.SelectedItem is ColorThemeJson)
            {
                ColorThemeJson s = (ColorThemeJson)cmbColorTheme.SelectedItem;

                Factory.Events.OnColorThemeLoaded(s.Name);
            }
        }
    }
}
